...
//        let localizedString = NSLocalizedString("en", bundle: bundle, comment: "")
//     
//        
//        
...
//    }
    public static func initialization(completion: @escaping (String, String, String) -> Void) {
        ...
        let preferredLanguage = Locale.preferredLanguages.first ?? "hi"
        ...
        DocumentReaderService.shared.initializeDatabaseAndAPI(progress: { state in
            ...
            case .downloadingDatabase(progress: let progress):
                let progressString = String(format: "%.1f", progress * 100)
                progressValue = bundle?.localizedString(forKey: "Downloading database: %d%%", value: nil, table: nil) ?? "Downloading database: \(progressString)%"
            case .initializingAPI:
                status = bundle?.localizedString(forKey: "Start Now", value: nil, table: nil) ?? "Start Now"
            case .completed:
                break
            case .error(let text):
                validationError = text
